...
class NodeCorrelationAnalyzer:
    ...
    def calculate_correlations(self) -> Tuple[float, List[float]]:
        """Calculate average correlation between pairs of sampled nodes from different tiers"""
        # Use existing method to get nodes by tier
        all_tiers = self.get_nodes_by_tier()
        
        # Filter to only include sampled nodes in each tier
        nodes_by_tier = {
            tier: nodes.intersection(self.sample_nodes) 
            for tier, nodes in all_tiers.items()
        }
        # Remove any tiers that have no sampled nodes
        nodes_by_tier = {
            tier: nodes 
        ...
        # Calculate correlations only between nodes from different tiers
        for tier1, tier2 in combinations(nodes_by_tier.keys(), 2):
            for node1 in nodes_by_tier[tier1]:
                for node2 in nodes_by_tier[tier2]:
        ...
        print(f"Successfully calculated correlations for {len(correlations)} pairs between different tiers, skipped {skipped_pairs} pairs")
...
def run_correlation_sweep(n: int, m: int, num_layers: int, x_values: np.ndarray,
    ...
    for i, x in enumerate(x_values):
        print(f"\nAnalyzing x = {x:.3f} ({i+1}/{len(x_values)})")
        
        try:
            network = SupplyNetwork(n=n, m=m, num_layers=num_layers, x=x)
            analyzer = NodeCorrelationAnalyzer(network, num_samples)
            
            # Run fully random sampling
            fully_random_avg, fully_random_corrs = analyzer.run_analysis(
                end_time, sampling_interval, sampling_method='fully_random')
            fully_random_avgs.append(fully_random_avg)
            fully_random_stds.append(np.std(fully_random_corrs) if fully_random_corrs else 0)
            ...
            if selected_tiers is not None:
                try:
                    ...
                    tier_restricted_stds.append(np.std(tier_random_corrs) if tier_random_corrs else 0)
                    ...
                    stratified_stds.append(np.std(strat_corrs) if strat_corrs else 0)
            ...
            print(f"Warning: {e}")
            fully_random_avgs.append(np.nan)
            fully_random_stds.append(np.nan)
            if selected_tiers is not None:
                tier_restricted_avgs.append(np.nan)
                tier_restricted_stds.append(np.nan)
                stratified_avgs.append(np.nan)
...
def main():
    ...
    selected_tiers = [0, 1]  # Only sample from tiers 0, 1, and 2
