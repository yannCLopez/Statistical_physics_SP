            ...
            for parent in nodes:
                for type_idx in range(self.m):
                    for child_idx in range(self.n):
                        ...
                        print("dictionary: ", self.children)
...
def test_functionality_check():
    ...
    print("Test 1: Simple network")
    network = SupplyNetwork(n=2, m=2, num_layers=4, x=0.8)
    
    # Add the vectorized method to the class
    SupplyNetwork.check_functionality_vectorized = check_functionality_vectorized
    
    # Test multiple edge state configurations
    num_tests = 10000
    mismatches = 0
    
    print(f"Running {num_tests} random state configurations...")
    
    for i in range(num_tests):
        # Randomly set edge states
        for edge in network.edges:
            network.edge_states[edge] = random.choice([0, 1])
        
        # Check both methods
        result_original = network.check_functionality()
        result_vectorized = network.check_functionality_vectorized()
        
        # Compare results
        if result_original != result_vectorized:
            mismatches += 1
            print(f"\nMismatch found in test {i+1}:")
            print(f"Original method: {result_original}")
            print(f"Vectorized method: {result_vectorized}")
            print("\nCurrent edge states:")
            for edge, state in network.edge_states.items():
                print(f"Edge {network.get_node_label(edge[0])} -> {network.get_node_label(edge[1])}: {state}")
            break
    
    if mismatches == 0:
        print("All tests passed! Both methods give identical results.")
    else:
        print(f"\nFound {mismatches} mismatches out of {num_tests} tests.")

# Run the test
if __name__ == "__main__":
    test_functionality_check()